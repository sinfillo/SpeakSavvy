...
LibraryWidget::LibraryWidget(BaseWindow *parent, BooksMap *bookMap)
    : parent_(parent), bookMap_(bookMap) {
  box_ = new QWidget;
  layout_ = new QGridLayout;
  box_->setLayout(layout_);

  auto screen_width =
      QGuiApplication::primaryScreen()->geometry().width() - 1000;
  auto screen_height =
      QGuiApplication::primaryScreen()->geometry().height() - 400;

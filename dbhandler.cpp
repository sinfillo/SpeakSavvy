...
DbHandler::~DbHandler() { networkManager_->deleteLater(); }

void DbHandler::getBooks() {
  connect(networkReply_, &QNetworkReply::readyRead, this,
          &DbHandler::networkReplyReadyRead);
  networkReplyReadyRead();
}

// void DbHandler::getOneBook(int i) {
//     connect(networkReply_, &QNetworkReply::readyRead, this,
//     &DbHandler::networkReadyReadOneBook); networkReadyReadOneBook(i);
// }
...
void DbHandler::networkReadyReadOneBook(int i) {
  ...
  QJsonArray json_array = json_doc.array();
  book_ = Book{
      i, json_array[i].toObject().value("name").toString().toStdString(),
